      ...
      <aside className="w-72 bg-surface-light border-r border-border flex flex-col shrink-0">
        <div className="p-4 border-b border-border">
          <Link to="/" className="block">
            ...
            <p className="text-xs text-text-dim mt-0.5">Browser-based BMAD Platform</p>
          </Link>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto px-2">
          {loading ? (
            <p className="text-center text-text-dim text-sm p-4">Loading...</p>
          ) : sessions.length === 0 ? (
            <p className="text-center text-text-dim text-sm p-4">No sessions yet</p>
          ) : (
